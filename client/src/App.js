...
import './App.css';
import romancheImage from './romanche.png';

function App() {
  const [text, setText] = useState('');
  const [direction, setDirection] = useState('romanche_to_francais');
  ...
  return (
    <div className="App">
      <h1>Traducteur Romanche-Français</h1>
      <textarea
        value={text}
      ...
      <select value={direction} onChange={(e) => setDirection(e.target.value)}>
        <option value="romanche_to_francais">Romanche vers Français</option>
        <option value="francais_to_romanche">Français vers Romanche</option>
      </select>
      <br />
